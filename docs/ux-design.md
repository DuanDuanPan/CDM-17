...
5) 分享：生成只读/可评论链接 → 系统校验密级（机密及以上禁止公开）→ 默认水印+遮罩开启且不可关闭 → 默认有效期 7 天（可调 1–30 天）/可失效 → 访问者写入访问记录，失效后返回明确错误码。
6) 访问记录：右侧“访问记录” → 过滤时间/事件/密级（查询强制时间窗+project_id）→ 导出 CSV/JSON；失败时提示分页降级；IP/UA 超 30 天仅显示哈希。
...
- 导出/分享对话框：左侧格式选择，右侧范围+水印+遮罩+有效期；遮罩不可关闭；遮罩节点>500 时提示“为保障可读性，建议选择子树或结构-only”；底部显示审计将记录的字段与错误码提示。
...
- R21 导出遮罩：选择敏感字段遮罩策略（隐藏/模糊）；默认水印/遮罩开启且不可关闭；范围下拉（全图/当前子树）；遮罩节点>500 时提示降级（结构-only 或子树）；导出失败重试/分片。
- R22 访问记录：右侧面板可过滤（时间/事件/密级，需选择时间窗）；导出 CSV/JSON；分页防超时；数据写入 append-only，IP/UA 超 30 天以哈希显示。
- R23 快速上手：/命令、粘贴预览（可编辑树）、AI 骨架（可回收）；快捷键提示悬浮层。
- R24 分享联动：生成链接时检查密级；默认水印+遮罩不可关；默认有效期 7 天（1–30 可调）；机密及以上禁用公开链接；一键失效；分享列表展示访问次数与最后访问时间与错误码。
